...
function analyze_sweeps_2_double(R_list, gd_param, preprocessed, param1, param2, varargin)
    ...
    title(strcat("Score vs. ", paramList{p1idx}{1}, " (", titleStr ,")"))
...
%Angle fresnel equations
%Rs = abs(((n1*cosd(phi)-n2*sqrt(1-((n1/n2)*sind(phi)).^2))./(n1*cosd(phi)+n2*sqrt(1-(n1/n2*sind(phi)).^2)))).^2;
%Rp = abs((n1*sqrt(1-(n1/n2 * sind(phi)).^2)-n2*cosd(phi))./(n1*sqrt(1-(n1/n2.*sind(phi)).^2)+n2.*cosd(phi))).^2;