...
%THIS IS A MODIFICATION TO SEPARATE THE POLARIZATION OUT
...
function analyze_sweeps_2_pol_sumOrders(R_list, gd_param, preprocessed, param1, param2, varargin)
    ...
    for i = 1:szx
        for j = 1:szy
            ...
            if ~isa(Rmat{i,j}, 'double')
                ...
                RmatE1{i,j} = sum(extractfield(s, 'eff1'));
                RmatE2{i,j} = sum(extractfield(s, 'eff2'));
            end
        end
    end
    ...
    RmatE1 = squeeze(cell2mat(RmatE1));
    RmatE2 = squeeze(cell2mat(RmatE2));
    ...
    normAngle = 0;
    %Boolean on whether to plot fresnel reflections or not
    plotAngle = 0;
    ...
    close all;
    p1 = plot(paramList{p1idx}{3}, RmatE1);
    hold on
    p2 = plot(paramList{p1idx}{3}, RmatE2, '--');

    if(strcmp(paramList{p1idx}{1}, "Angle") && plotAngle == 1)
        %Compute the fresnel reflection!
        %FIXME: This currently is hardcoded, because we don't usually save
        %this information. REMEMBER TO CHANGE if needed!
        n1 = 1;
        ...
        plot(paramList{p1idx}{3}, Rs, '-.r')
        plot(paramList{p1idx}{3}, Rp, '-.r')
    end
    ...
    lgd = legend(p1,string(paramList{p2idx}{3}));
    title(lgd, strcat("s-polarization, ", paramList{p2idx}{1}, "(", paramList{p2idx}{2},")"))
    ah1=axes('position',get(gca,'position'),'visible','off');
    lgd2 = legend(ah1, p2,string(paramList{p2idx}{3}));
    title(lgd2, strcat("p-polarization, ", paramList{p2idx}{1}, "(", paramList{p2idx}{2},")"))
