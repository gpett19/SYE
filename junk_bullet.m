...
d1 = 1.11:0.01:1.15; % grating period in x2 direction
d2 = 1.11:0.01:1.15; % grating period in x3 direction
%Radius of pyrs
r = 1;


h = 0.88:0.01:0.92; % grating height
wlr = 1.50:0.02:1.60;
%h = 1;
%wavelength = 1.55;
...
m_max = 7; % maximum diffraction order index
...
for l1 = 1:L1
    ...
    stratum.stripe{1}.c1 = -(sqrt((2/L1^3)*l1^3 - (3/L1^2)*l1^2 + 1))/2*r(k);
    stratum.stripe{2}.c1 = -stratum.stripe{1}.c1;
    % Set first and second block' boundaries (c2) on positive side:
    stratum.stripe{2}.block{1}.c2 = -(sqrt((2/L1^3)*l1^3 - (3/L1^2)*l1^2 + 1))/2*r(k);
...
x_limit = [-0.5*h(1),-1.5*d1(1),-1.3*d1(1);1.5*h(1),1.5*d1(1),1.3*d1(1)];
